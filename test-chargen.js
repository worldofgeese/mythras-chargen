...
// Test: CSE combat-style authority matches inline culture combat styles
{
  ...
  (ref.styles || []).forEach(style => {
    (grouped[style.culture] ||= []).push({
      ...
      traits: style.traits,
      ...(style.restrictions ? { restrictions: style.restrictions.startsWith('only available') ? style.restrictions : `only available to ${style.restrictions}` } : {}),
  ...
  if (ref.authority?.source === 'Combat Styles Encyclopedia' &&
    ...
    fail('CSE combat-style authority is out of sync with inline data',
      JSON.stringify({ source: ref.authority?.source, legacyStatus: legacy.status, legacyDoNotUse: legacy.do_not_use_for_app_generation, missing, extra, diffs }));
  }
}
